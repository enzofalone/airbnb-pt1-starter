...
class Booking {
  ...
  static async newBooking({newBooking, listing, user}) {
    const newBookingRequiredFields= ["startDate", "endDate"];

    newBookingRequiredFields((field) => {
    ...
    const query = `
        ...
        VALUES 
          ...
          ($3)::date,
          $4,
          CEIL((($3)::date - ($2)::date + 1)) * 
          ($5 * 1.1)),
          $6,
          (SELECT id FROM users WHERE users.username = $7))
        RETURNING id, 
          start_date AS "startDate",
          end_date AS "endDate",
          guests,
      ...
      const result=  await db.query(query, [newBooking.paymentMethod || 'card', Date.toString(newBooking.startDate), Date.toString(newBooking.endDate), newBooking.guests || 1, newBooking.price, listing.id, user.username])
      return result;
  }
